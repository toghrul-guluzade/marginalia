...
/** Center + right panes of the workspace: toolbar, PDF viewer, annotation sidebar.
 *  Remounted (keyed) per docId, so its state resets cleanly when switching docs. */
export default function DocPane({ docId }: { docId: string }) {
  ...
  // Load the PDF blob from IndexedDB.
  useEffect(() => {
    let cancelled = false;
  ...
  // Persist scroll/zoom/page; restore scroll after load.
  useEffect(() => {
    ...
    <div className="flex min-w-0 flex-1 flex-col">
      ...
      <div className="flex min-h-0 flex-1">
        <main className="min-h-0 min-w-0 flex-1">
          {loadError ? (
            <div className="flex h-full items-center justify-center text-dim">{loadError}</div>
          ) : pdfUrl ? (
            <PDFViewer
              ref={viewerRef}
              url={pdfUrl}
            ...
            <div className="flex h-full items-center justify-center text-dim">Loading document…</div>
          )}
        </main>
